...
class RequestHandlingMiddleware(BaseHTTPMiddleware):
    async def authorize(self, request):
        ...
        if n_key == "su":
            ...
            # ] = await NeubilityApiAccessKey.get_api_access_key_by_value(api_key=n_key)
            # if len(docs) < 1:
            # return 401
            # doc = docs[0]
    ...
    async def set_state(self, request: Request):
        ...
        handler_obj = list(
            filter(
                lambda el: (el.path_regex.search(request.state.path) != None)
                and list(el.methods)[0] == request.state.method,
                self.app.app.routes,
            )
        )
    ...
    async def dispatch(
        ...
        if not set_state:
            return JSONResponse(
                {"result": None, "processTime": 0, "message": "Not found", "code": 404},
